...
/// One row of `gameobject_model_tags`: a gameobject model display id, an optional
/// human label, and up to 3 free-form tags used for searching.
#[derive(Debug, Clone, Serialize, Deserialize, FromRow)]
pub struct GameObjectModelTag {
    pub displayId: u32,
...
/// Search gameobject models by tag. Matches the query as a substring against any
/// of the 3 tag columns or the name, and returns the matching display ids.
#[tauri::command]
pub async fn search_gameobject_model_tags(
    state: State<'_, DbState>,
    app: tauri::AppHandle,
    debug: State<'_, DebugState>,
    search: String,
    limit: Option<i64>,
) -> Result<Vec<GameObjectModelTag>, String> {
